...
int n, m;
vector<int> alist[1001];    // Adjacency list for the original graph
vector<int> t_alist[1001];  // Adjacency list for the transposed graph
...
int main() {
    ...
    for (int i = 0; i < m; i++) {
        int x, y;
        cin >> x >> y;
        alist[x].push_back(y);  // Original graph: edge from x to y
        t_alist[y].push_back(x);  // Transposed graph: edge from y to x
    }
